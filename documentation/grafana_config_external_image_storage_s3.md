...
| `region`           | String |         | Set the AWS region                                              | Any AWS region e.g `us-east-1`                   |
| `bucket`           | String |         | Set the S3 bucket name                                          |                                                  |
| `bucket_url`       | String |         | Bucket URL for S3. e.g. - <https://grafana.s3.amazonaws.com/>   |                                                  |
| `path`             | String |         | Optional extra path inside bucket                               | Valid path inside the S3 bucket                  |
| `access_key`       | String |         | Access key for the aws account. Works along with `secret_key`.  | Valid AWS access key                             |
| `secret_key`       | String |         | Secret key for the aws account. Works along with `access_key`.  | Valid AWS secret key                             |
